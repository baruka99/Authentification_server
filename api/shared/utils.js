...
var randomString = function () {
    const caracteres = "abcde123456fghijklABCDEFGHIJKLMNOPQRSTUVWXYZmnopqrstuvwxyz0789"
    let finalString = "";

    for (let i = 0; i < 15; i++) {
        finalString += caracteres[Math.random() * 62 | 0]
    }
    return finalString;
}
...
var sha256 = function (value) {
    var toUTF8octetString = function (str) {
        ...
        function codePointToUtf8BinaryNotation(codePoint) {
            // this  function will help us to convert codePoint into binary code
            const str = codePoint.toString(2);
            // toString() that is the base 10 ,base 2 (toString(2)), will help us showing the binary, 
            //if hexa, it could be base 16
            const len = str.length; // the number 0 and 1 of the codePoint converted into binary base

            // for the case of // 0xxxxxxx
            if (len <= 7) {
    ...
    // we convert our utf8 String binary to the bloc of 512 bits using this
    // function
    ...
    // console.log(toUTF8octetString(value))
    // make512Blocks(toUTF8octetString(value))[0].forEach((x) => console.log(`0x${x.toString(16).padStart(8, 0)}`))
    ...
    return sha(toUTF8octetString(value))
